...
void check_setpgid(pid_t pid, pid_t pgid);
void check_kill(pid_t pid, int sig);

void check_sigemptyset(sigset_t *set);
void check_sigaddset(sigset_t *set, int signum);
void check_sigprocmask(int num, const sigset_t *set, sigset_t *oldset);
...
void eval(char *cmdline) 
{
    // I have to make the parent must use sigprocmask to block SIGCHLD signals before it forks the child,
    // and then unblock these signals, again using sigprocmask after it adds the child to the job list by calling addjob.
    // Clearly if my shell makes a child process then by default that child will also be a member of the foreground process group.
    // ctrl c will send a signal to the shell and every process created, which isn't what I want 
   
    // Thus before the execve but after the fork, the child process should call setpgid(0, 0),
    ...
    if (argv[0] == NULL) {//if theres empty lines just return
        return;            
   }
    
    if (!builtin_cmd(argv)) { //check to see if builtin cmd arg
        check_sigemptyset(&mask);                    
        check_sigaddset(&mask, SIGCHLD);             
        check_sigprocmask(SIG_BLOCK, &mask, NULL);  
        
        
        // Child
        if ((pid = check_fork()) == 0) {
            check_setpgid(0, 0);                       //identical to child id?
            check_sigprocmask(SIG_UNBLOCK, &mask, NULL);    
        ...
        if (!bg) {  
            ...
            check_sigprocmask(SIG_UNBLOCK, &mask, NULL);                 // Unblocks SIGCHLD signal
...
int builtin_cmd(char **argv) 
{    
    //simple function implementation if user has typed built in then I need to execute
    if (!strcmp(argv[0], "quit")) { //if first arg is equivalent to quit
        exit(0); //then exit
    }
    else if (!strcmp(argv[0], "jobs")) { //if first arg is jobs then list
        listjobs(jobs);
        return 1;
    }
    else if (!strcmp(argv[0], "bg")) { //if bg then do bgfg
        do_bgfg(argv);
        return 1;
    }
...
{
    if (argv[1] == NULL) {                                     
        printf("%s command requires PID or %%jobid argument\n", argv[0]); //if second argument then need  PID or job id argument
        return;
    }
    ...
    if(strcmp(argv[0], "bg") == 0) {
        ...
        check_kill(-givenjob->pid, SIGCONT);                              // Send SIGCONT signal to entire group of the given job
    } else {
...
void sigchld_handler(int sig) 
{
    // In sigchld_handler one call to sigchld handler
    ...
    while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {    // WE MUST REAP THE ZOMBIE CHILD REAP I SAY
        jobid = pid2jid(pid);
...
void sigint_handler(int sig) 
{
    ...
    if (pid != 0) {
        ...
        check_kill(-pid, sig); // signals to the entire foreground process group
...
//Functions for system call errors I defined
...
void check_sigprocmask(int num, const sigset_t *set, sigset_t *oldset) {
    if (sigprocmask(num, set, oldset) < 0) {
